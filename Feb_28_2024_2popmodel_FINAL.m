...
% res=rate of inducing resistance
...
cost_flag = input(prompt); % Stores value answered to prompt
...
if cost_flag == 1
    path = ['Output_fmincon_', num2str(treat_flag), 'uM_L1_AutoTracking_2pop_' , num2str(clN)];
elseif cost_flag == 2
    path = ['Output_fmincon_', num2str(treat_flag), 'uM_L2_AutoTracking_2pop' , num2str(clN)];
end
if exist(path, 'dir') ~= 7
    mkdir(path)
end
...
lb2=[0.001 1e-4 1e-4 1e-4 1e-4];
ub2=[0.15 0.7 0.6 0.6 1];
...
%number of computers to send to--> 10 worked best for this computer
M=10;
...
figure()
[t,y] = ode23s(@(t,x) two_pop_model(t,x,best_fit_params),time,[1 0]);
z=1:length(time);
...
% S'=r_s*S*(1-(S+R)/K)-d*S*(1-exp(-1*gamma_1*t))-res*S*(1-exp(-1*gamma_2*t))
% R'=res*S*(1-exp(-1*gamma_2*t))+r_r*R*(1-(S+R)/K)-e*d*R*(1-exp(-1*gamma_1*t))
