%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Fitting 2-population induced resistance model to dose-specific time   %
...
% S+R using a multistart fmincon algorithm. The model used is:          %
%  S'=r_s*S-d*S*(1-exp(-1*gamma_1*t))-res*S*(1-exp(-1*gamma_2*t))       %
%  R'=res*S*(1-exp(-1*gamma_2*t))+r_r*R-e*d*R*(1-exp(-1*gamma_1*t))     %
% Parameters fit per dose are:                                          %
% 1) r_S = growth rate of sensitive (S) cells                           %
% 2) d = death rate of S                                                %
% 3) res = rate at which S transition to resistance (R)                 %
% 4) r_R = growth rate of R (assumed to be <= r_S)                      %
% 5) e=epsilon: where death rate of R d_R is = e*d_S (assumed to be     %
...
%                                                                       %
% This version allows for pre-existing resistance                       %
% Run time over 10 pools: 757.810984 - 923.791552 secs                  %
%                                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Set pre-existing resistance IC as desired
R0 = 0.01; % fraction of pre-existing resistant cells
S0 = 1-R0; 
...
fulltime=data(:,1);
prompt = "What dose do you want to fit?\n Enter enter 3.2, 1, 0.32, .1, .032, or 0 for uM: ";
...
if cost_flag == 1
    path = ['Output_', num2str(treat_flag), ...
        'uM_L1_2pop_preexisting_' , num2str(clN)];
elseif cost_flag == 2
    path = ['Output_', num2str(treat_flag), ...
        'uM_L2_2pop_preexisting_' , num2str(clN)];
end
if exist(path, 'dir') ~= 7
    mkdir(path)
end
...
%% Parameter fitting
% ORDER OF PARAMS: r_s, d, res, r_r, e,
% Parameter bounds for multi start and fmincon
%     r_s    d     res   r_r   e 
lb = [0.001  1e-4  1e-4  1e-4  1e-4];
ub = [0.15   0.7   0.6   0.6   1];
...
fun = @(z)objective(z,time,Amean,cost_flag,S0,R0);

% Multi-start minimization across Mpool parallel pools
Mpools = 10; % number of parallel pools
...
[t,y] = ode23s(@(t,x) two_pop_model(t,x,best_fit_params_list(1,:)),...
    time,[S0 R0]);
z=1:length(time);
allpops(z)=y(z,1)+y(z,2);
figure;
...
plot(time, y(:,1), 'Color', 'g')
plot(time, y(:,2), 'Color','magenta')
%plot(time, replicates, 'Color','k')
hold off;
xlabel('Time (hr)')
ylabel('Normalized Cell Count')
title("2 Population model of resistance of " +treat_flag + "uM Vemurafenib in COLO858")
legend('Data Average','S+R Best Fit', 'S', 'R','location','best')
...
%% FUNCTIONS                                                           %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function cost = objective(p,time,ydata,flag,S0,R0) % Cost to minimize
    % Solves model assuming all cells initiall sensitive
    [t, y] = ode23s(@(t,x) two_pop_model(t,x,p),time,[S0,R0]);
...
function xp=two_pop_model(t,x,p) % ODE model
