%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Display best-fits and suboptimal parameter sets from ALTERATE         %
% 3-population induced resistance model.                                %
...
load Output_fmincon_0.032uM_L1_AutoTracking_3pop_12192329/output.mat
best_pars_0032 = best_fit_params_list; 
best_obj_0032 = best_fit_objective_list; 
clear best_fit_params_list best_fit_objective_list

load Output_fmincon_0.32uM_L1_AutoTracking_3pop_12192135/output.mat
best_pars_032 = best_fit_params_list; 
best_obj_032 = best_fit_objective_list; 
clear best_fit_params_list best_fit_objective_list

load Output_fmincon_3.2uM_L1_AutoTracking_3pop_12192036/output.mat
best_pars_32 = best_fit_params_list; 
best_obj_32 = best_fit_objective_list; 
clear best_fit_params_list best_fit_objective_list
...
function xp = three_pop_model(t,x,p)
    %ORDER OF PARAMS: r_s,d_1, alpha, frac, r_r, d_2
    r_s=p(1);
    d=p(2);
    alpha=p(3);
    frac=p(4);
    ...
    xp(1) = r_s*S-d*S*(1-exp(-1*gamma_1*t))-alpha*S*(1-exp(-1*gamma_2*t));
    xp(2) = frac*alpha*S*(1-exp(-1*gamma_2*t));
    xp(3) = (1-frac)*alpha*S*(1-exp(-1*gamma_2*t))+r_r*R-e*d*R*(1-exp(-1*gamma_1*t));
