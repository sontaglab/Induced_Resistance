%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Fitting ALTERNATE 3-population induced resistance model to            %
% dose-specific time course data for COLO858 cells treated Vemurafenib  %
...
function xp = three_pop_model(t,x,p)
    %ORDER OF PARAMS: r_s,d_1, q, reS, r_r, d_2
    r_s=p(1);
    d=p(2);
    q=p(3);
    res=p(4);
    ...
    xp(2) = res*q*S*(1-exp(-1*gamma_2*t));
    xp(3) = (1-res)*q*S*(1-exp(-1*gamma_2*t))+r_r*R-e*d*R*(1-exp(-1*gamma_1*t));
